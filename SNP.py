...
class data_set_SNP:
    def __init__(self, main_folder, condition_folder, data_set_prefix, deconvolution_method="BCL",
        ...
        self.Spikes.astype(np.float)
        self.Centers = np.loadtxt(
            main_folder + condition_folder + data_set_prefix + "_all_cells_centers_cut_ordered.dat")
        ...
        np.save(arr= self.numbered_burst,file = main_folder + condition_folder + data_set_prefix + "_" +deconvolution_method+"_"+ "intro_removed_"+ str(remove_intro) + "_bursts.npy")
        self.burst_stats_df = pd.concat([self.burst_stats_df, self.spikes_stats], 1)
        print(self.burst_stats_df)
    ...
    def print_sync_frames_and_cell_number(self):
        ...
        print("frame_number:", str(self.Spikes[1]))
    ...
    def detect_bursts(self):
        self.smoothed_spikes = ndimage.gaussian_filter1d(self.Spikes.astype(np.float), axis=1, sigma=20)
...
def combine_SNPs(main_folder, results_folder, remove_intro=True, deconvolution_method="BCL"):
    ...
    for j, f in enumerate(folders):
        print(f)
        data_set_list = [os.path.basename(x) for x in
                         glob.glob(main_folder + f + "/" + "*_all_cells_centers_cut_ordered.dat")]
        ...
        for i, d in enumerate(data_set_prefix):
            print(d)
            d_S = data_set_SNP(main_folder=main_folder, condition_folder=f + "/", data_set_prefix=d,
                               deconvolution_method=deconvolution_method, remove_intro=remove_intro)
            data_set_df[i] = d_S.burst_stats_df
        condition_df[j] = pd.concat(data_set_df)
        print(condition_df[j].groupby(["data_set_name", "Rearing_condition", "age"], as_index=False).mean())
    ...
    total_df.to_pickle(
        results_folder + "SNP_table_" + deconvolution_method + "_remove_intro-" + str(remove_intro) + ".pkl")
